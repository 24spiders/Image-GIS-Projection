...
from .utils import getWKT_PRJ, read_cfg
...
def single_img2gis(cfg):
    ...
    try:
        ...
        exit("Error: DSM does not seem to have CRS information. Chcek and try again")
    ...
    for col in cols:
        if col[0] == 'geometry':
            ...
            w.field(*col)                  # add other field normally
    # + extra fields
...
def batch_img2gis(cfg):
    ...
    for col in cols:
        if col[0] == 'geometry':
            ...
            w.field(*col)                  # add other fields normally
    # + extra fields
    w.field('distance_to_img_center', 'F', decimal=3)
    ...
    for _, row in tqdm(points_df.iterrows(), total=len(points_df)):
        ...
        xlim = (np.amin(points[:, 0]), np.amax(points[:, 0]))
        ylim = (np.amin(points[:, 1]), np.amax(points[:, 1]))
        center = np.array([(xlim[0] + xlim[1]) // 2, (ylim[0] + ylim[1]) // 2]).reshape(-1, 2)  # TODO: This is different than above
        points = np.vstack([center, points])
